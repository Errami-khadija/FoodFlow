<header class="bg-white shadow-sm border-b border-gray-100 px-4 lg:px-8 py-4 flex items-center justify-between flex-shrink-0">
     <div class="flex items-center gap-4">
      ...
      <div>
       <h2 id="page-title" class="text-xl lg:text-2xl font-bold text-gray-800">  {{ $pageTitle ?? 'Dashboard' }}</h2>
       <p class="text-sm text-gray-400">  {{ $pageSubtitle ?? "Welcome back! Here's what's happening today." }}</p>
      </div>
     </div>
     ...
     <div class="flex items-center gap-3">
      <button class="relative p-2 rounded-xl hover:bg-gray-100 transition-colors">
       ...
       @php
    $name = $restaurant->owner->name ?? 'Unknown';
